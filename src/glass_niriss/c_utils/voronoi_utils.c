// Various voronoi related functions
...
float default_sn_func(
    ...
    for (i=0; i<n; i++) {
        if (index[i]) {
            sum_signal += signal[index[i]];
            sum_noise += pow(noise[index[i]],2);
        }
    };
    sum_noise = pow(sum_noise, 0.5);
    return sum_signal/sum_noise;
}

void voronoi_tessellation(
    int n,
    float * x,
    float * y,
    float * xnode,
...
def _cvt_equal_mass(x, y, signal, noise, xnode, ynode, pixelsize, quiet, sn_func, wvt):
    ...
    for it in range(xnode.size):  # Do at most xnode.size iterations
        ...
        # print (len(xnode), good, len(good), scale[good], scale[~good])
        # import matplotlib.pyplot as plt
        # plt.scatter(np.arange(len(scale[good])), scale[good])
        # plt.show()
