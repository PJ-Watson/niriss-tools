...
import numpy as np
import spectres
from bagpipes import config, filters, plotting, utils
...
class modified_model_galaxy(object):
    ...
    def _get_wavelength_sampling(self):
        ...
        if self.spec_wavs is None:
            self.max_wavs = [
                (self.filter_set.min_phot_wav / (1.0 + max_z)),
                1.01 * self.filter_set.max_phot_wav,
                10**8,
            ]
            ...
            if (
                self.spec_wavs[0] > self.filter_set.min_phot_wav
                ...
                self.R = [config.R_other, config.R_spec, config.R_phot, config.R_other]
                ...
                self.R = [config.R_other, config.R_phot, config.R_spec, config.R_other]
        ...
        for i in range(len(self.R)):
            if i == len(self.R) - 1 or self.R[i] > self.R[i + 1]:
                while x[-1] < self.max_wavs[i]:
                    x.append(x[-1] * (1.0 + 0.5 / self.R[i]))

            else:
                while x[-1] * (1.0 + 0.5 / self.R[i]) < self.max_wavs[i]:
                    x.append(x[-1] * (1.0 + 0.5 / self.R[i]))
    ...
    def _get_R_curve_wav_sampling(self, oversample=4):
        ...
        if "resolution_p0" in list(self.model_comp):
            x = R_curve[:, 0]
            x = 2.0 * (x - (x[0] + (x[-1] - x[0]) / 2.0)) / (x[-1] - x[0])
    ...
    def _get_index_spec_wavs(self, model_components):
        ...
        sampling = np.round(np.mean([min, max]) / 5000.0, 2)

        return np.arange(min, max * (1.0 + config.max_redshift), sampling)

    def update(self, model_components, cont_only=False, rm_line=None):
    ...
    def _calculate_full_spectrum(self, model_comp, cont_only=True, rm_line=None):
        ...
        if self.nebular and not cont_only:
            ...
            # # plt.plot(self.wavelengths, self.spectrum_full)
            # line_fluxes = dict(zip(config.line_names, em_lines))
            # fig, axs = plt.subplots(1,2, sharex=True, sharey=True)
            # axs[0].plot(self.wavelengths,
            # spectrum_bc,
            # # self.nebular.spectrum(grid, t_bc,
            # #         model_comp["nebular"]["logU"]),
            # )
            # axs[0].set_title(line_fluxes["H  1  6562.81A"])
            ...
            if rm_line is not None:
                ...
                for rm in rm_line:
                    ...
                    # Assume line_wav_shift is 0, I'm too tired to sort this out properly
            ...
            # axs[1].plot(self.wavelengths, spectrum_bc,
            # # self.nebular.spectrum(grid, t_bc,
            # #         model_comp["nebular"]["logU"]),
            # )
            # plt.show()
    ...
    def _calculate_photometry(self, redshift, uvj=False):
        ...
        if uvj:
            phot = self.uvj_filter_set.get_photometry(
                self.spectrum_full, redshift, unit_conv=unit_conv
            )

        else:
            phot = self.filter_set.get_photometry(
                self.spectrum_full, redshift, unit_conv=unit_conv
            )
    ...
    def _calculate_spectrum(self, model_comp):
        ...
        fluxes = spectres.spectres(self.spec_wavs, redshifted_wavs, spectrum, fill=0)
    ...
    def _calculate_uvj_mags(self):
        ...
        self.uvj = -2.5 * np.log10(self._calculate_photometry(0.0, uvj=True))
