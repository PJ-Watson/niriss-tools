...
from glass_niriss.grism.specgen import modified_model_galaxy
...
class BagpipesSampler(object):
    def __init__(self, fit_instructions, veldisp=500.0):
    ...
    def sample(self, param_vector, cont_only=False, rm_line=None, **model_kwargs):
        ...
        if self.model_gal is None:
            self.model_gal = modified_model_galaxy(new_comps, **model_kwargs)
        # else:
...
class RegionsMultiBeam(MultiBeam):
    ...
    def __init__(self, binned_data, pipes_dir, run_name, beams, **multibeam_kwargs):
