...
if __name__ == "__main__":
    ...
    for a, (line_name, line_lam) in zip(
        ...
        line_lam /= 1e4
        spacing = 0.015
        xlims = [line_lam - spacing, line_lam + spacing]
        for i, f in enumerate(nirspec_dir.glob("**/*.fits")):
            dat = QTable.read(f, "SPEC1D")
            dat = dat[
                (dat["wave"].to(u.micron).value >= xlims[0] * (1 + obj_z_nirspec))
                & (dat["wave"].to(u.micron).value <= xlims[-1] * (1 + obj_z_nirspec))
            ]
            ...
            a.plot(
                np.array(dat["wave"] / (1 + obj_z_nirspec)),
                flux_cgs.value,
                drawstyle="steps-mid",
                c=colour,
                # label=f"NIRSpec, {resolution}" if i == 0 else None,
    ...
    for _, a in axes.items():
        ...
        a.set_xlabel(r"$\lambda_{\rm{restframe}}\,(\mu m)$")
