...
if __name__ == "__main__":
    ...
    try:
        niriss_ext_cat = Table.read(
            stage_cat_dir / "full_external_em_lines_revised.fits"
        )
    except:
        lines = {
        ...
        niriss_ext_cat["N_LINES"] = 0
        # niriss_ext_cat["NAME_LINES"] =
            ...
            if np.isfinite(row["Z_NIRISS"]):
                ...
                for l, w_rf in lines.items():
                    ...
                    if not check_coverage(
                        ...
                        n_good_lines += 1
                        name_lines_list.append(l)
                # print (";".join(name_lines))
                name_lines = ";".join(name_lines_list)
                niriss_ext_cat["N_LINES"][i] = n_good_lines
            if row["Z_FLAG"] <= 2:
                for l, w_rf in lines.items():
                    niriss_ext_cat[f"flux_{l}"][i] = 0.0
                    niriss_ext_cat[f"err_{l}"][i] = 0.0
                niriss_ext_cat["Z_NIRISS"][i] = np.nan
                niriss_ext_cat["N_LINES"][i] = 0
                # niriss_ext_cat[]
                name_lines = ""
            # print (name_lines)
            name_lines_all.append(name_lines)
        niriss_ext_cat["NAME_LINES"] = name_lines_all
        print(len(niriss_ext_cat))
        print(len(name_lines_all), "end")

        niriss_ext_cat.write(
            stage_cat_dir / "full_external_em_lines_revised.fits", overwrite=True
        )
    ...
    try:
        ...
        for l in lines:
            aanda_cols.extend([f"flux_{l}", f"err_{l}"])
