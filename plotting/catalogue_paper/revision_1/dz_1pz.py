...
if __name__ == "__main__":
    ...
    full_cat[secure & (full_cat["Z_NIRISS"] >= 7.5)]["ID", "zmed_prev"].pprint_all()
    # exit()
    print(
        np.sum(secure),
        np.sum(secure & spec),
    ...
    for i, (z_name, z_cat_name, idx) in enumerate(
        ...
        # scatter = np.nanstd(
        scatter = stats.mad_std(
            (
                full_cat["Z_NIRISS"][secure & idx]  # & ~offset]
                - full_cat[z_cat_name][secure & idx]  # & ~offset]
            )
        )
        str_print = f"{scatter:.1E}".split("E")
        str_print = rf"{str_print[0]}\times 10^{{{int(str_print[-1])}}}"
        axs[0, i].text(
            0.1,
            0.9,
            rf"$\sigma_{{z}}={str_print}$",
            va="top",
        ...
        cluster_idx = (full_cat["Z_NIRISS"] >= 0.2) & (full_cat["Z_NIRISS"] <= 0.4)
        print(np.nanmedian(dz[secure & idx]))
        print(np.nanstd(dz[secure & idx]))
        print(stats.median_absolute_deviation(dz[secure & idx]))
        print(stats.mad_std(dz[secure & idx]))
        print(stats.biweight_scale(dz[secure & idx]))
        for c_idx in [cluster_idx, ~cluster_idx]:
            print("Cluster", np.nanmedian(full_cat["Z_NIRISS"][secure & idx & c_idx]))
            print(np.nanmedian(dz[secure & idx & c_idx]))
            print(np.nanstd(dz[secure & idx & c_idx]))
            print(stats.median_absolute_deviation(dz[secure & idx & c_idx]))
            print(stats.mad_std(dz[secure & idx & c_idx]))
            print(stats.biweight_scale(dz[secure & idx & c_idx]))
        # exit()
        if z_cat_name == "zmed_prev":
            ...
            for j, (source, colname) in enumerate(
                ...
                str_print = f"{np.nanmedian(dz[secure & idx & selection]):.1E}".split(
                    "E"
                )
                str_print = rf"{str_print[0]}\times 10^{{{int(str_print[-1])}}}"
                # print (str_print)
            ...
            print(
                "F_OUT",
                np.nansum(dz[(secure | tentative) & idx] > 0.15)
                / np.nansum(secure & idx),
            )
            str_print = f"{np.nanmedian(dz[secure & idx]):.1E}".split("E")
            str_print = rf"{str_print[0]}\times 10^{{{int(str_print[-1])}}}"
            axs[1, i].text(
                0.1,
                0.9,
        ...
        full_cat[
            ...
            "z_nirspec",
            "id_msa_1324",
        ][(secure & idx) & (np.abs(dz) > 0.02)].pprint_all()
    # ax.hist(
    ...
    # )
    print(
        full_cat["ID", "Z_NIRISS", "Z_FLAG", "zmed_prev"][(full_cat["Z_NIRISS"] >= 7)]
    )
    # ax.scatter(
    ...
    axs[0, 0].set_xticks(z_ticks[:-1], z_ticks_labels[:-1], minor=True)
    axs[0, 1].set_xticks(z_ticks, z_ticks_labels, minor=True)
    axs[0, 1].set_yticks(z_ticks, z_ticks_labels, minor=True)
    for a in axs.flatten()[:2]:
        a.set_xticks(
    ...
    # axs[0, 1].set_yticks([0.1, 1.0], ["0.1", "1.0"], minor=False)
    axs[1, 1].set_xlabel(r"$z_{\rm{phot}}$")
    axs[1, 0].set_xlabel(r"$z_{\rm{spec}}$")
    axs[0, 0].set_ylabel(r"$z_{\rm{NIRISS}}$")
    axs[1, 0].set_ylabel(r"$\delta z / (1+z)$")
    ...
    print(
        np.sum(
            (secure | tentative)
            # (secure)
            & (full_cat["Z_NIRISS"] >= cluster_low)
            & (full_cat["Z_NIRISS"] < cluster_high)
            & ~spec
        )
    )
    print(
