...
if __name__ == "__main__":
    ...
    with fits.open(img_path) as img_hdul:
        ...
        for a_i, (ax, (z_low, z_high), icon, size, colour) in enumerate(
            ...
            print((1e3 * u.kpc / kpc_scale).to(u.deg).value)
            # ob = AnchoredHScaleBar(
            #     size=(1e3*u.kpc / kpc_scale).to(u.deg).value,
            #     label=r"$\rm{10\ kpc}$",
            #     loc=4,
            #     frameon=False,
            #     pad=0.1,
            ...
            #     textkw=dict(color="k"),
            #     ax=ax
            #     # bbox_transform=ax.get_transform("world"),
            #     # transform=ax.get_transform("world"),
            # )
            # ax.add_artist(ob)
            ax.errorbar(
                3.568 + ((1e3 * u.kpc / kpc_scale).to(u.deg).value / 2),
                -30.415,
                xerr=((1e3 * u.kpc / kpc_scale).to(u.deg).value / 2),
                transform=ax.get_transform("world"),
                capsize=3,
                ecolor="k",
            )
            ax.text(
                # "test",
                3.568 + ((1e3 * u.kpc / kpc_scale).to(u.deg).value / 2) - 0.002,
                -30.412,
                r"1\,Mpc",
                # (3.568 + ((1e3 * u.kpc / kpc_scale).to(u.deg).value / 2), -30.39),
                transform=ax.get_transform("world"),
                ha="center",
                # xycoords="data",
            )
