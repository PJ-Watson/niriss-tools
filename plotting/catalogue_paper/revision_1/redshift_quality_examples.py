...
if __name__ == "__main__":
    ...
    for o, (obj_id, obj_class) in enumerate(zip(obj_list_full.ravel(), obj_list_class)):
        # print(dir(subfigs.ravel()[0]))
        # gs = subfigs.ravel()[o].add_gridspec(2, 2, width_ratios=[1, 3])
        gs = subfigs.ravel()[o].add_gridspec(1, 4, width_ratios=[1.0, 1.5, 3, 1])

        row = ext_cat[ext_cat["ID_NIRISS"] == obj_id]

        ax_text = subfigs.ravel()[o].add_subplot(gs[0])
        ax_text.annotate(
            rf"({chr(97+o)})\ \ "
            + "\n"
            + rf"ID: {obj_id}"
            + "\n"
            + rf"$z={row["Z_NIRISS"].value[0]:.3f}$"
            + "\n"
            + rf"{obj_class}",
            (1.0, 0.5),
            rotation="horizontal",
        ...
        ax_img = subfigs.ravel()[o].add_subplot(gs[1])
        ax_img.imshow(cimg, origin="lower")
        ax_img.set_xticks([])
        ax_img.set_yticks([])

        # ax_img.set_ylabel(
        #     rf"({chr(97+o)})\ \ {obj_class}"
        #     + "\n"
        #     + rf"ID: {obj_id}"
        #     + "\n"
        #     + rf"$z={row["Z_NIRISS"].value[0]:.3f}$",
        ...
        # continue
        axs_oned = subfigs.ravel()[o].add_subplot(gs[2])
        with fits.open(oned_path) as oned_hdul:
            ...
            for i, hdu in enumerate(oned_hdul[1:]):
                ...
                axs_oned.plot(
                    ...
                    # drawstyle="steps-mid",
                    c="k",
                    label="Best-fit Model" if i == 2 else "_",
                )
