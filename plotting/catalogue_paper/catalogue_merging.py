...
if __name__ == "__main__":
    ...
    z_cat_name = "merged_z_cat_4.fits"
    ...
    ground_idx = np.zeros(len(full_z_cat), dtype=bool)
    ground_idx[
        np.where(
            (~niriss_idx)
            & (~nirspec_idx)
            & (~nircam_idx)
            & (np.isfinite(full_z_cat["z_ground"]))
            & (~full_z_cat["z_ground"].mask)
        )
    ] = True
    ...
    z_ground = np.array(full_z_cat[ground_idx]["z_ground"])
