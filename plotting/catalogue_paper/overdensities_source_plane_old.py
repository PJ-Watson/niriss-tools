...
if __name__ == "__main__":
    ...
    with fits.open(img_path) as img_hdul:
        ...
        for z_low, z_high in [[1.09, 1.11], [1.32, 1.37], [1.85, 1.9], [2.63, 2.67]]:
            # print (z_low)
            # continue
            # exit()
            fig, axs = plt.subplots(
                ...
                figsize=(
                    ...
                    plot_utils.aanda_columnwidth * 1.5,
                ),
                constrained_layout=True,
            ...
            z_med = np.median([z_low, z_high])
            axs[0].set_title(f"z={z_med:.3f}")
            ...
            plot_utils.sky_plot(
                lens_tab, ax=axs[0], dec_col="Dec", facecolor="blue", label="Observed"
            )
            plot_utils.sky_plot(
                lens_tab,
                ax=axs[1],
            ...
            # exit()
            Z *= len(lens_tab)
            scale_corr = (1 / ((1 * u.deg).to(u.arcsec) * kpc_scale) ** 2).to(
                1 / u.Mpc**2
            )
            ...
            # print (scale_corr)
            # print (cnt.levels*scale_corr)
            import matplotlib as mpl

            # cmap = mpl.cm.plasma
            norm = mpl.colors.Normalize(
                vmin=cnt.levels[0] * scale_corr.value,
                vmax=cnt.levels[-1] * scale_corr.value,
            )
            ...
            axs[0].coords[0].set_ticks_visible(False)
            ...
            plt.savefig(save_dir / f"od_source_plane_{z_med:.3f}.pdf")
